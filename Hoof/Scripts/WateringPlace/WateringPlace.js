var WateringPlace = function (config) {
    ...
    var getWateringPlaceModel = function () {
        var id = $('#addWateringPlaceModal').data('id');
    ...
    $("#addWateringPlaceModal").on("hidden.bs.modal", function () {
        clearModal();
    });
    ...
    $('#save').on('click', function () {
        ...
        if (isFormValid) {
            ...
            $.removeData($('#addWateringPlaceModal'), 'id');
        }
    });

    $('.editWateringPlace').on('click', function () {
        ...
        $('#addWateringPlaceModal').data('id', id).modal('show');
    });

    $('.deleteWateringPlace').on('click', function () {
        var row = $(this).closest('tr');
        var id = row.find('td:nth-child(1)').text().trim();
        var section = row.find('td:nth-child(2)').text().trim();
        var district = row.find('td:nth-child(3)').text().trim();
        ...
        $('#confirmDeleteModalBody').text('Czy na pewno chcesz usunąć wodopój z oddziału ' + section + ', rewir ' + district + ', leśnictwo ' + forestry + '?');
