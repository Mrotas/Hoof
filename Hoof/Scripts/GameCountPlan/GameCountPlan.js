var GameCountPlan = function (config) {
    ...
    var validateForm = function (row) {
        ...
        var count = row.find('td:nth-child(5)').find('input');
        if (count.val() === '') {
    ...
    var getGameHuntPlanModel = function (row, isNew) {
        var id, gameType, gameId, kind, kindName, subKind, subKindName, gameClass, gameClassName, count;
        if (isNew) {
            ...
            count = row.find('td:nth-child(5)').find('input').val();
        } else {
            ...
            count = row.find('td:nth-child(7)').text();
        }
